...
var SlapDoor = function (opts) {
    opts = opts || {};
    this.src = opts.src || '';
    this.minHeight = isNaN(opts.minHeight) ? 0 : opts.minHeight;

    this.angle = isNaN(opts.angle) ? 0.5 : opts.angle;
    this.margin = isNaN(opts.margin) ? 0 : opts.margin;
    this.isRight = !!opts.isRight;
...
SlapDoor = EventTrigger.extend(SlapDoor);

SlapDoor.prototype.setSymbol = function (img) {
    this.symbol = img;
};

SlapDoor.prototype.loadImage = function () {
    var instance = this;
    ...
    $(img).on('load', function () {
        ...
        instance.trigger('load', instance);
    });

    img.src = src;
};

SlapDoor.prototype.setImage = function (img) {
    this.img = img;
    this.height = Math.max(this.minHeight, img.height) / this.resolution;
    this.offset = (this.height * this.angle) / 2;
};

SlapDoor.prototype.drawCanvas = function () {
    var img = this.img;
    ...
    if (symbol) {
        ...
        ctx.drawImage(
            symbol,
            (this.isRight ? 0 : (w - cut)) + (cut - symbol.width) / 2,
            (h - symbol.height) / 2
        );
    }
 ...
 * const
 * ========================================= */
//var IMG_ROOT = 'http://tsune-ni-idm.com/img';
var IMG_ROOT = 'http://mat1.gtimg.com/fashion/mengyi/meijibangyang/33/';
var SECTION_MIN_HEIGHT = 680;

var IS_PC = $(document.body).is('.pc');
...
(function setupSectionHeight() {
    ...
    var height = Math.max($(window).height(), SECTION_MIN_HEIGHT);
    //$sections.css('height', height + 'px');
    //$sections.css('height', '1227px');
    $("#A,.header__inner,.header-content").css('height', '1227px');
})();
...
(function setupDoor() {
    var ANGLE = 0;//瑙掑害
    var MARGIN = 450;
    var RESOLUTION = IS_PC ? 1 : 2;

    var LEFT_SRC = [ IMG_ROOT, 'bg_door_organizm.jpg' ].join('/');
    var MM_SRC = [ IMG_ROOT, 'bg_mm.jpg' ].join('/');
    var RIGHT_SRC = [ IMG_ROOT, 'bg_door_flower.jpg' ].join('/');
    ...
    var $doorPrompt = $('#door-prompt');
    var $copyright = $('#copyright');

    var ww = $window.width();
    var wh = $window.height();
    var sectionHeight = Math.max(wh, SECTION_MIN_HEIGHT);
    
    var isRendered = false;
    var isFirstView = true;

    var symbol = new Image();
    $(symbol).on('load', render);
    symbol.src = [
        IMG_ROOT,
        (IS_PC ? 'bg_logo.png' : 'bg_logo_sp.png')
    ].join('/');

    var left = new SlapDoor({
        src: LEFT_SRC,
        isRight: false,
    ...
    right.on('load', render);

    function render() {
        if (isRendered || !left.img || !mm.img || !right.img || !symbol.width) {
            return;
        }

        left.setSymbol(symbol);
        mm.setSymbol(symbol);
        right.setSymbol(symbol);

        var leftCanvas = left.drawCanvas();
        var mmCanvas = mm.drawCanvas();
        var rightCanvas = right.drawCanvas();
        ...
        $(leftCanvas).css({
            ...
            marginTop: (-left.height / 2) + 'px',
            marginRight: (-left.offset) + 'px'
        });
        ...
        $(rightCanvas).css({
            ...
            marginTop: (-right.height / 2) + 'px',
            marginLeft: (-right.offset) + 'px'
        });
        ...
        //spacer.style.height = (sectionHeight * 2) + 'px';
        spacer.style.height = '1905px';
        $header.before(spacer);

        function updateDoorPosition() {
            var y = Math.max(0, $window.scrollTop());
            var x = (y / sectionHeight) * (left.offset + ww / 2);
            var oo = 100/$window.scrollTop();
              console.log(y)
            leftCanvas.style.transform = leftCanvas.style.webkitTransform = 
                //'translate(' + [ '0px', -5*y + 'px' ] + ')';
                'translate(' + [ -x + 'px', 0 + 'px' ] + ')';
            rightCanvas.style.transform = rightCanvas.style.webkitTransform = 
                //'translate(' + [ '0px', -5*y + 'px' ] + ')';
                'translate(' + [ x + 'px', 0 + 'px' ] + ')';
            mmCanvas.style.transform = mmCanvas.style.webkitTransform = 
                'translate(' + [ '0px', -5*y + 'px' ] + ')';
            mmCanvas.style.opacity = oo;

            $header.css({
                position: (sectionHeight < y) ? 'absolute' : 'fixed',
                top: (sectionHeight < y) ? ('680px') : '0px'
            });
            $headerTitle.css({
                position: (sectionHeight < y) ? 'absolute' : 'fixed',
                //top: (sectionHeight < y) ? (sectionHeight + (wh / 2) + 'px') : '50%'
            });
            $c_A_t1.css({
                position: (sectionHeight < y) ? 'absolute' : 'fixed',
            });
            ...
            if (y > 0 && isFirstView) {
                ...
                if (IS_PC) {
                    $doorPrompt.fadeOut();
                    $headerContent.fadeIn().addClass('Cur');
                } else {
                    $copyright.fadeOut();
                }
            } else if (y === 0 && !isFirstView) {
                isFirstView = true;
                if (IS_PC) {
                    $doorPrompt.fadeIn();
                    $headerContent.fadeOut().removeClass('Cur');
                } else {
                    $copyright.fadeIn();
                }
            }
        ...
        $window.on('scroll', updateDoorPosition);
        setTimeout(updateDoorPosition);
...
(function setupScrollTracker() {
    ...
    var $gnav = $('#gnav');
    var $gnavLink = $gnav.find('a');

    var ticker = new Ticker({ auto: true, clock: 20 });
    ...
    scrollTracker.on('changeSection', function (e) {
        $gnavLink.removeClass(ACTIVE_CLASS_NAME);
        $gnavLink.eq(e.index).addClass(ACTIVE_CLASS_NAME);
    });
    ...
    if (IS_PC) {
        $gnavLink.on('click', function (e) {
            e.preventDefault();
            var href = this.href;
            var matchData = href.match(/#([^#]+)$/);
            if (!matchData) {
                return;
            }
            var id = matchData[1];
            scrollTracker.jumpToSection(id, 500);
        });
    }
})();
...
$('.DB_tab25').DB_tabMotionBanner({
  ...
  motion:
  {
    ...
    c_B_a1:{left:50,opacity:0,speed:500,delay:100},
    c_B_b2:{top:50,opacity:0,speed:500,delay:500},

    c_B_a2:{left:50,opacity:0,speed:500,delay:100},
    c_B_b2:{top:50,opacity:0,speed:500,delay:500},

    c_B_a3:{left:50,opacity:0,speed:500,delay:100},
    c_B_b3:{top:50,opacity:0,speed:500,delay:500},

    c_B_a4:{left:50,opacity:0,speed:500,delay:100},
    c_B_b4:{top:50,opacity:0,speed:500,delay:500},
...
function closeBox(ct){
  $("#"+ct).fadeOut(100);
}
/*  |xGv00|c6375a440be315f1c844c9dd584c266f */